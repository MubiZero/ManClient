...
1. Создайте в payom шаблон, который несёт код в `{text-1}`, например «ManClient: код подтверждения {text-1}». Модерация занимает часы; отредактировать шаблон после создания нельзя.
...
1. Открыть `Записи`, переключить «День» и «Список», проверить поиск по имени и телефону.
