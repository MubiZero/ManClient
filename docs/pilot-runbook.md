...
1b. Переключить «Неделя»: семь колонок с общим окном часов, закрытые дни серые, клик по заголовку дня открывает этот день. Свободные получасы кликабельны только когда в филиале один специалист или один выбран в фильтре — иначе форма не знала бы, к кому записывать.
