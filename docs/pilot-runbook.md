...
1c. Перетащить визит в сетке дня на другое время (шаг 15 минут) и в колонку другого специалиста: время меняется, в истории записи появляется `booking.rescheduled` с прежним специалистом. Отказ («специалист занят», «нельзя в прошлое») показывается над сеткой, и запись остаётся на месте. Перенос другому специалисту доступен владельцу и администратору; сотрудник переставляет только свои визиты по времени. Мышь не единственный путь — форма переноса в карточке записи работает как раньше.
