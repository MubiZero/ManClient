...
- [ ] **Step 1: Написать падающие тесты на прайс**
...
- [ ] **Step 3: Добавить модели `PlanPrice` и `SubscriptionInvoice` и аддитивную миграцию**

`PlanPrice` уникален по `[plan, period]`. `SubscriptionInvoice` хранит `amountDiram` слепком, а не ссылкой на прайс.

- [ ] **Step 4: Реализовать чтение и запись прайса**

- [ ] **Step 5: Написать падающие тесты на выставление счёта**
...
- [ ] **Step 6: Добавить `PLATFORM_PAYMENT_CARD` в валидируемое окружение**
...
- [ ] **Step 7: Реализовать сервис счетов**

- [ ] **Step 8: Собрать экран прайса в платформенном бэкофисе**
...
- [ ] **Step 1: Написать падающие тесты на приём и разбор чека**
...
- [ ] **Step 1: Написать падающий тест на переходы и на то, что задача идемпотентна**
...
- [ ] **Step 3: Реализовать задачу и зарегистрировать её в общем списке**
...
- [ ] **Step 4: Слать предупреждения через существующую очередь уведомлений бизнеса**
...
- [ ] **Step 3: Реализовать страницу и компоненты**
...
- [ ] **Step 4: Добавить баннер `GRACE` в layout кабинета**

Баннер называет дату и что именно выключится. Он не перекрывает работу и закрывается на сессию.

- [ ] **Step 5: Прогнать компонентные и браузерные тесты**
...
- [ ] **Step 1: Описать в runbook подписки, проверку чеков платформой и что делать при спорной оплате**

Отдельно — как выдать бизнесу срок вручную и почему `subscriptionEndsAt = null` означает бессрочно.

- [ ] **Step 2: Прогнать статические проверки**

Run: `pnpm prisma validate && pnpm lint && pnpm typecheck`

- [ ] **Step 3: Прогнать полный сюит**

Run: `set -a; . ./.env; set +a; pnpm test`

- [ ] **Step 4: Прогнать браузерные сценарии**

Run: `pnpm test:e2e`

- [ ] **Step 5: Проверить диффом, что в git не попали карта платформы и суммы из тестов**
