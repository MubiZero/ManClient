...
>
> Расхождения с планом: Компонент назван `src/features/dashboard/bookings/booking-actions-panel.tsx` (в плане — `booking-mutation-form.tsx`), тест — `tests/unit/dashboard/booking-actions-panel.test.tsx`.
...
- [x] **Step 1: Write failing command-alias integration tests**
...
- [x] **Step 2: Run the new tests and verify the current default help response fails the assertions**

Run: `pnpm test -- tests/integration/integrations/business-bot-handler.test.ts`

- [x] **Step 3: Add the slash-command aliases to the existing switch**
...
- [x] **Step 1: Write failing renderer tests for two-column dates, three-column time slots, and separate navigation rows**
...
- [x] **Step 2: Run the renderer test and verify the missing helper causes failure**

Run: `pnpm test -- tests/unit/integrations/conversation-renderer.test.ts`

- [x] **Step 3: Add a small grid helper without changing the existing one-action-per-row default**

Expose a dedicated helper used only for short labels such as dates and slots. Long business names stay one per row.

- [x] **Step 4: Write failing customer-flow tests for a cancellation escape and booking-summary editing**
...
- [x] **Step 5: Run the customer-flow test and verify it fails for missing actions**

Run: `pnpm test -- tests/integration/integrations/telegram-booking-flow.test.ts`

- [x] **Step 6: Implement only the required action branches and views**
...
- [x] **Step 7: Re-run focused Telegram tests**
...
- [x] **Step 1: Write failing payment-page tests for visit context and a pending hold indicator**
...
- [x] **Step 2: Run the new payment-page test and verify it fails because staff, date/time, and hold state are not rendered**

Run: `pnpm test -- tests/unit/public-payment/payment-page.test.tsx`

- [x] **Step 3: Extend the public payment projection with only staff and branch timezone**

Do not expose card, internal IDs, or other customer data.

- [x] **Step 4: Render a client-side, second-aligned remaining-hold label and an explicit expired state**
...
- [x] **Step 5: Re-run the payment-page test**
...
- [x] **Step 1: Write failing component tests for confirmation copy and safe dismiss actions**
...
- [x] **Step 2: Run the component test and verify it fails because the forms submit immediately**

Run: `pnpm test -- tests/unit/dashboard/booking-mutation-form.test.tsx`

- [x] **Step 3: Implement a focused client mutation form around the existing server actions**
...
- [x] **Step 4: Replace `datetime-local` with availability-backed date and slot selection**
...
- [x] **Step 5: Extend the browser regression to verify cancel confirmation and slot-based reschedule**

The test must open the dialog, dismiss once, then confirm; it must select an available slot rather than fill arbitrary time.

- [x] **Step 6: Run focused unit and browser checks**
...
- [x] **Step 1: Generate Prisma client and run static checks**

Run: `pnpm prisma generate && pnpm lint && pnpm typecheck`

- [x] **Step 2: Run the full Vitest suite against the isolated local database**

Run: `pnpm test`

- [x] **Step 3: Run the relevant Playwright flows with a seeded local database**

Run: `pnpm playwright test tests/e2e/booking-operations.spec.ts tests/e2e/public-booking.spec.ts`

- [x] **Step 4: Inspect the diff, commit only W0 files, and report the external production smoke still required**
