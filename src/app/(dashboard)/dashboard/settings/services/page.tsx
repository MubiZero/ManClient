...
function serviceValues(formData: FormData) { return { branchId: String(formData.get("branchId") ?? ""), name: String(formData.get("name") ?? ""), description: String(formData.get("description") ?? ""), durationMinutes: String(formData.get("durationMinutes") ?? ""), bufferBeforeMinutes: String(formData.get("bufferBeforeMinutes") ?? "0"), bufferAfterMinutes: String(formData.get("bufferAfterMinutes") ?? "0"), amountSomoni: String(formData.get("amountSomoni") ?? ""), prepaymentMode: String(formData.get("prepaymentMode") ?? "") as "" | "FULL" | "DEPOSIT" | "NONE", depositPercent: String(formData.get("depositPercent") ?? ""), depositSomoni: String(formData.get("depositSomoni") ?? ""), staffIds: formData.getAll("staffIds").map(String), resourceIds: formData.getAll("resourceIds").map(String), isPublished: formData.get("isPublished") === "true" }; }
