...
/**
 * The approved template is "{text-1}: код подтверждения {text-2}", and payom substitutes nothing it was
 * not given: a placeholder we forget ships to the customer as literal "{text-2}", and one we invent is
...
describe("phone verification SMS", () => {
  ...
  it("puts who is asking first and the code second", () => {
    ...
    expect(buildPhoneVerificationSms("123456", "Барбершоп Алиф")).toEqual({
      ...
      variables: { "text-1": "Барбершоп Алиф", "text-2": "123456" },
    });
  });
